...
const Posts = ({ history }) => {
	...
	<div className="max-w-md w-full space-y-8 bg-white bg-opacity-90 px-10 pb-6 pt-4 rounded-lg border-4 border-white">
	  ...
	  <img
		...
		<h2 className="mt-6 text-center text-3xl font-extrabold text-red-700">Post About Your Favorite Travel Spots!</h2>
	  ...
	  <form className="mt-8 space-y-6" 
		...
		<div className="rounded-md shadow-sm -space-y-px">
		  <div>
			...
			/>
			{errors.PostTitle && <p style={{ color: "red" }}>{errors.PostTitle}</p>}
		  </div>
		  </div>
		  <div>
			...
			/>
			{errors.PostBody && <p style={{ color: "red" }}>{errors.PostBody}</p>}
		  </div>
<button
              type="submit"
              onClick={Posts}
